import { $, $$, clearElement, CONFIRM_WORDS_COUNT, copyToClipboard,
    createElement, formatDate, formatDateFull, formatTime,
    IMPORT_WORDS_COUNT, onInput, setAddr, toggle, toggleFaded, triggerClass
} from "./Utils.js";
...
class View {
  ...
  constructor(mnemonicWords) {
    ...
    // paste invoice link start ton:// link will be open send popup
    ...
    //import and creat new wallet DPS wallet
    $("#start_createBtn").addEventListener("click", () =>
      this.sendMessage("showScreen", { name: "created" })
    );
    $("#start_importBtn").addEventListener("click", () =>
      this.sendMessage("showScreen", { name: "import" })
    );
    ...
    $("#main_buyBtn").addEventListener("click", () => {
      window.open(
        "https://exchange.mercuryo.io/?currency=TONCOIN&address=" +
          this.myAddress,
        "_blank"
      );
    });
    ...
    $("#import_alertBtn").addEventListener("click", () => {
      this.showAlert({
        title: "Too Bad",
        message:
          "Without the secret words, you can't restore access to your wallet.",
        buttons: [
          {
            label: "CANCEL",
            callback: () => {
    ...
    $("#createdContinueButton").addEventListener("click", () =>
      this.sendMessage("createPrivateKey")
    );

    $("#backup_continueBtn").addEventListener("click", () => {
      const currentTime = Date.now();
      if (currentTime - this.backupShownTime < 60000) {
        // 1 minute
        this.showAlert({
          title: "Sure done?",
    ...
    $("#readyToGo_continueBtn").addEventListener("click", () =>
      this.sendMessage("showMain")
    );
    ...
    $("#main_settingsButton").addEventListener("click", () =>
    ...
    $("#sendButton").addEventListener("click", () =>
      this.onMessage("showPopup", { name: "send" })
    );
    $("#swapButton").addEventListener("click", () =>
      this.onMessage("showPopup", { name: "swaping" })
    );

    $("#multipleButton").addEventListener("click", () =>
      this.onMessage("showPopup", { name: "multiple" })
    );
    ...
    $("#menu_magic").addEventListener("click", () => {
      ...
      const isTurnedOn = $("#menu_magic .dropdown-toggle").classList.contains(
        "toggle-on"
      );
    ...
    $("#menu_proxy").addEventListener("click", () => {
      ...
      this.sendMessage(
        "onProxyClick",
        $("#menu_proxy .dropdown-toggle").classList.contains("toggle-on")
      );
    });

    $("#menu_about").addEventListener("click", () => this.showPopup("about"));
    $("#menu_changePassword").addEventListener("click", () =>
      this.onMessage("showPopup", { name: "changePassword" })
    );
    $("#menu_backupWallet").addEventListener("click", () =>
      this.sendMessage("onBackupWalletClick")
    );
    $("#menu_delete").addEventListener("click", () => this.showPopup("delete"));

    $("#receive_showAddressOnDeviceBtn").addEventListener("click", () =>
      this.onShowAddressOnDevice()
    );
    $("#receive_invoiceBtn").addEventListener("click", () =>
      this.onCreateInvoiceClick()
    );
    $("#receive_shareBtn").addEventListener("click", () =>
      this.onShareAddressClick(false)
    );
    $("#receive .addr").addEventListener("click", () =>
      this.onShareAddressClick(true)
    );
    $("#receive_closeBtn").addEventListener("click", () => this.closePopup());

    $("#invoice_qrBtn").addEventListener("click", () =>
      this.onCreateInvoiceQrClick()
    );
    $("#invoice_shareBtn").addEventListener("click", () =>
      this.onShareInvoiceClick()
    );
    $("#invoice_closeBtn").addEventListener("click", () =>
      this.showPopup("receive")
    );

    $("#invoiceQr_shareBtn").addEventListener("click", () =>
      this.onShareInvoiceClick()
    );
    $("#invoiceQr_closeBtn").addEventListener("click", () =>
      this.showPopup("invoice")
    );

    $("#transaction_sendBtn").addEventListener("click", () =>
      this.onTransactionButtonClick()
    );
    $("#transaction_closeBtn").addEventListener("click", () =>
      this.closePopup()
    );
    ...
    $("#send_btn").addEventListener("click", (e) => {
      ...
      if (
        !toAddressString.toLowerCase().endsWith(".ton") &&
        !toAddressString.toLowerCase().endsWith(".t.me") &&
        !toAddress
      ) {
      ...
      const isExchange =
        toAddress && exchangeAddresses.indexOf(toAddress.toString(false)) > -1;

      if (isExchange) {
        if (!comment) {
          $("#notify").innerText =
            "Exchange require text comment or (MEMO) for deposit";
          triggerClass($("#notify"), "faded-show", 2000);
          return;
        }
        ...
        if (comment && needEncryptComment) {
          $(
            "#notify"
          ).innerText = `Can't encrypt comment or memo when depositing on exchange`;
          triggerClass($("#notify"), "faded-show", 2000);
          return;
        }
      }
    ...
    // button for multiple send

    // closed buttos tag and coding
    $("#send_closeBtn").addEventListener("click", () => this.closePopup());
    $("#swaping_closeBtn").addEventListener("click", () => this.closePopup());
    $("#multiple_closeBtn").addEventListener("click", () => this.closePopup());
    ...
    $("#sendConfirm_okBtn").addEventListener("click", () =>
      this.onMessage("showPopup", { name: "enterPassword" })
    );
    ...
    $("#signConfirm_okBtn").addEventListener("click", () =>
      this.onMessage("showPopup", { name: "enterPassword" })
    );
    ...
    $("#processing_closeBtn").addEventListener("click", () =>
      this.closePopup()
    );
    ...
    $("#about_version").addEventListener("click", (e) => {
      if (e.shiftKey) {
        this.showAlert({
          ...
          message:
            "You can switch back the network by clicking on the version with the Shift key pressed",
          buttons: [
            {
              label: "I'M SURE",
              callback: () => {
        ...
        this.showAlert({
          title:
            "Are you sure you want to switch between clear console/debug mode?",
          message:
            "You can switch back the clear console by clicking on the version with the Alt key pressed",
          buttons: [
            {
              label: "I'M SURE",
              callback: () => {
    ...
    $("#changePassword_cancelBtn").addEventListener("click", () =>
      this.closePopup()
    );
    ...
    $("#delete_okBtn").addEventListener("click", () =>
  ...
  setUpdating(updating) {
    $("#updateLabel").innerText = updating
      ? "⏳ Updating..."
      : "DPS Balance updated just now";
  }
  ...
  setBalance(balance, txs) {
    try {
      if (!balance || !(balance instanceof BN)) {
        console.warn("⚠️ Invalid balance received in setBalance:", balance);
        balance = new BN(0); // fallback
      }
      ...
      if (!Array.isArray(txs)) {
        console.warn("⚠️ Invalid txs list in setBalance:", txs);
        txs = [];
      }
  ...
  addDateSeparator(dateString) {
    const text = `📅 Transaction History – ${dateString}`;
    $("#transactionsList").appendChild(
      createElement({ tag: "div", clazz: "date-separator", text })
    );
  }
        ...
        createElement({
          ...
          child: isReceive
            ? [
                ...
                createElement({ tag: "span", text: " Amount Received" }),
                createElement({
                  tag: "span",
                  clazz: "tx-from",
                  text: " from 👇:",
            ...
            : [
                ...
                createElement({ tag: "span", text: " 💎 amount send " }),
                createElement({ tag: "span", clazz: "tx-from", text: "to:" }),
              ],
        }),
        setAddr(
          createElement({ tag: "div", clazz: ["tx-addr", "addr"] }),
          addr
        ),
  ...
  onTransactionClick(tx) {
    ...
    $("#transactionAmount").innerText =
      (isReceive ? "+" + amountFormatted : amountFormatted) + " 💎";
    $("#transactionFee").innerText = fromNano(tx.fee) + " transaction fee";
    $("#transactionSenderLabel").innerText = isReceive
      ? "Sender wallet ID"
      : "Recipient";
    setAddr($("#transactionSender"), addr);
    toggle(
      $("#transactionCommentLabel"),
      !!tx.comment || !!tx.encryptedComment
    );
  ...
  onTransactionButtonClick() {
    this.onMessage("showPopup", {
      name: "send",
      toAddr: this.currentTransactionAddr,
    });
  }
  ...
  setMyAddress(address) {
    ...
    // ✅ Also update another element (e.g. in main section)
    const walletEl = document.getElementById("walletDisplay");
    if (walletEl) {
      walletEl.innerText = address;
    }

    // ✅ Store in a separate variable for future use (e.g., in API URLs)
    const walletDisplay = address;  // now you can use this anywhere in JS
  ...
  onShareAddressClick(onyAddress) {
    const data = onyAddress
      ? this.myAddress
      : TonWeb.utils.formatTransferUrl(this.myAddress);
    const text = onyAddress
  ...
  getInvoiceLink() {
    ...
    return TonWeb.utils.formatTransferUrl(
      this.myAddress,
      amount,
  ...
  onShareInvoiceClick() {
    $("#notify").innerText = copyToClipboard(this.getInvoiceLink())
      ? "Transfer link copied to clipboard"
      : "Can't copy link";
    triggerClass($("#notify"), "faded-show", 2000);
  }

  // RECEIVE INVOICE QR CODE POPUP
  ...
  onMessage(method, params) {
    switch (method) {
      ...
      case "setIsMagic":
        ...
        $("#menu_magic .dropdown-toggle").classList.toggle(
          "toggle-on",
          isTurnedOn && !IS_FIREFOX
        );
        $("#menu_telegram").classList.toggle(
          "menu_telegram-show",
          isTurnedOn && !IS_FIREFOX
        );
        break;
      ...
      case "sendCheckCantPublicKey":
        ...
        $(
          "#notify"
        ).innerText = `To encrypt a message, the destination wallet must have at least one outgoing transfer`;
        triggerClass($("#notify"), "faded-show", 3000);
        break;
      ...
      case "sendCheckCantPayFee":
        ...
        $("#notify").innerText = `Estimated fee is ~${fromNano(
          params.fee
        )} TON`;
        triggerClass($("#notify"), "faded-show", 3000);
        break;
      ...
      case "decryptedComment":
        if (
          this.currentOpenTransaction &&
          this.currentOpenTransaction.hash === params.hash
        ) {
        ...
        switch (params.name) {
          ...
          case "send":
            ...
            toggle(
              ...
              !this.isLedger ? "flex" : false
            );
            $("#toWalletInput").focus();
            break;
          case "sendConfirm":
            $("#sendConfirmAmount").innerText =
              fromNano(new BN(params.amount)) + " TON";
            setAddr($("#sendConfirmAddr"), params.toAddress);
            $("#sendConfirmFee").innerText = params.fee
              ? "Fee: ~" + fromNano(new BN(params.fee)) + " TON"
              : "";

            toggle(
              $("#sendConfirmNotEncryptedNote"),
              !params.needEncryptComment
            );
            ...
            break;
          case "signConfirm":
            $("#signConfirmText").innerText = params.isConnect
              ? "Do you want to connect your wallet to this page?"
              : "Do you want to sign:";
            toggle($("#signConfirmData"), !params.isConnect);
            toggle($("#signConfirmAlert"), !params.isConnect);

            const hex =
              params.data.length > 48
                ? params.data.substring(0, 47) + "…"
                : params.data;
...
if (window.top == window && window.console) {
    ...
    console.log('%c%s', 'font-size: 14px;', localizedSelfXssAttention[1]);
